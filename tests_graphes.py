...
class TestsGraphesOrientes(unittest.TestCase):
    ...
    def test_arcs(self):
        ...
        a8 = Arc(s2, s5)
        a9 = Arc(s4, s5)
        self.assertEqual(a1, a7)
        self.assertNotEqual(Arc(s2, s1), a1)
        self.assertNotEqual(a3, a4)
        self.assertEqual(a3, a8)
        self.assertEqual(a6, a9)
        arcs = {a1, a2, a3, a4, a5, a6}
        self.assertEqual(6, len(arcs))
        valuations = set(map(lambda arc: arc.valuation(), list(arcs)))
        self.assertEqual({1.0, 7.0, -2.0, -1.0}, valuations)
        arcs.add(a7)
        self.assertEqual(6, len(arcs))
        valuations = set(map(lambda arc: arc.valuation(), list(arcs)))
        self.assertEqual({1.0, 7.0, -2.0, -1.0}, valuations)
        arcs.discard(Arc(s1, s2))
        arcs.add(a7)
        self.assertEqual(6, len(arcs))
        valuations = set(map(lambda arc: arc.valuation(), list(arcs)))
        self.assertEqual({5.0, 1.0, 7.0, -2.0, -1.0}, valuations)
        self.assertEqual({Arc(s1, s2), Arc(s2, s4), Arc(s2, s5), Arc(s5, s2), Arc(s5, s1), Arc(s4, s5)}, arcs)
